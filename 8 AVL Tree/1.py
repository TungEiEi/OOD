    ...
    def burntree(self, root):
        ...
        while queue:
            ...
            for node in nodes:
                node.Burn = True
                visited.add(node)
                for friend in [node.left, node.right, node.parent]:
                    if friend and friend not in visited:
                        next_nodes.append(friend)
