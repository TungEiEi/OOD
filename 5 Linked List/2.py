...
for i in inp:
    ...
    if i[:2] == "A ":
        ...
        if not D.insert(int(index), data):
        ...
        node, ind = D.remove(i[2:])
        if node is None:
            print("Not Found!")
        else:
            print(f"removed : {node} from index : {ind}")
        print("linked list :",D)
        print("reverse :",D.str_reversed())
